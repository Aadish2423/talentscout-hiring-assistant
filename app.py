...
### Phase 4 — Closing
Thank the candidate. Summarize what was collected. Inform them a recruiter will reach out within 2-3 business days. Wish them well.
...
def render_session_summary():
    ...
    q_answered = st.session_state.question_count
...
if not st.session_state.messages and not st.session_state.ended:
    ...
    if user_input:
        ...
        if len(user_input) < 2 or not any(c.isalpha() for c in user_input):
            st.session_state.toast = "⚠️ Please enter a valid response"
            st.rerun()
...
if st.session_state.ended:
    ...
    with col2:
        if st.session_state.candidate:
            st.download_button(
                "📥 Download Profile",
                data=json.dumps(st.session_state.candidate, indent=2),
                file_name=f"candidate_{st.session_state.candidate.get('name', 'profile').replace(' ', '_')}.json",
    ...
    if user_input:
        ...
        if is_exit(user_input):
            ...
            farewell = "Thank you for speaking with TalentScout! 🎯 It was great learning about your background. Our team will be in touch if there's a suitable match. Best of luck with your job search! 👋"
        ...
        # Check if assistant ended
        end_phrases = ["recruiter will reach out", "reach out within", "best of luck", "all the best", "goodbye", "farewell"]
        if any(p in response.lower() for p in end_phrases) and len(st.session_state.messages) > 8:
            st.session_state.ended = True
            try:
                save_candidate_record(st.session_state.candidate, st.session_state.messages)
            except Exception:
